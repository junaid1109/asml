                    ...
                    <form action="{{ route('admin.menus.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="label" class="form-control @error('label') is-invalid @enderror" 
                                   placeholder="e.g., Services, About, Contact" value="{{ old('label') }}" required>
                            @error('label')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="route_name" class="form-control @error('route_name') is-invalid @enderror">
                                ...
                                <option value="{{ $routeName }}" @if(old('route_name') == $routeName) selected @endif>
                                    {{ $label }} ({{ $routeName }})
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" name="url" class="form-control @error('url') is-invalid @enderror" 
                                   placeholder="e.g., https://example.com" value="{{ old('url') }}">
                            @error('url')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
